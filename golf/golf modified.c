...
int main(void)
{
    ...
    while (!WindowShouldClose())
    {
        ...
        switch (state)
        {
            ...
            case STATE_PLAYING:
            {
                ...
                if (IsMouseButtonReleased(MOUSE_BUTTON_LEFT) && startedplaying == 1)
                {
                    ...
                    if(Vector2Length(speed) > MAX_SPEED) speed = Vector2Scale(Vector2Normalize(speed), MAX_SPEED);
                ...
                // Check if level failed (more than 5 shots)
                if (shotcount > 5 && Vector2Length(speed) < STOP_THRESHOLD && !hasWon && !hitPond)
                {
        ...
        switch (state)
        {
            ...
            {
                ClearBackground(RAYWHITE);
                DrawText("LEVEL FAILED!", GetScreenWidth() / 2 - 150, GetScreenHeight() / 2 - 100, 40, RED);
                DrawText("You exceeded 5 shots!", GetScreenWidth() / 2 - 180, GetScreenHeight() / 2 - 20, 20, MAROON);
                DrawText("Press R to Restart Level", GetScreenWidth() / 2 - 180, GetScreenHeight() / 2 + 60, 20, BLUE);
                DrawText("Press Q to Quit to Menu", GetScreenWidth() / 2 - 180, GetScreenHeight() / 2 + 100, 20, BLUE);
                break;
            }
