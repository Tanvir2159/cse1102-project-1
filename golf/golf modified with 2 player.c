...
static void DrawBannerText(const char *text, float y, float fontSize, Color edge, Color outline)
{
...
static void MoveLevel5Obstacles(float dt)
{
    for (int i = 0; i < 3; i++)
    {
...
static bool Level6BallHitSpikes(void)
{
    for (int i = 0; i < 4; i++)
    {
...
static Vector2 PipePoint(float distance)
{
...
static void UpdateLevel6Pipe(void)
{
...
static void MoveThroughPipe(float dt)
{
...
static void UpdateLevel7Portals(void)
{
    for (int i = 0; i < 2; i++)
    {
...
static void HitHazard(const char *message, Sound sound)
{
    hitHazard = true;
...
static void DrawLevel2Course(void)
{
...
static void DrawLevel5Course(void)
{
    for (int i = 0; i < 3; i++)
    {
        DrawTexturePro(
...
static void DrawLevel6Course(void)
{
...
static void DrawLevel7Course(void)
{
    for (int i = 0; i < 6; i++)
    {
        DrawTexturePro(
...
static void TypeIntoName(char *name)
{
...
static void ConfirmPlayerNames(void)
{
...
int main(void)
{
    ...
    Font font = LoadFontEx("Assets/Fonts/Minecrafter-MA3Dw.ttf", 96, NULL, 0);
    ...
    while (!WindowShouldClose() && !quitRequested)
    {
            ...
            case STATE_PLAYING:
            {
                ...
                if (textDisplayTimer <= 0.0f && hasWon)
                {
                    if (gameMode == GAME_MODE_TWO_PLAYER)
                    {
                        ...
                        else if (playerExhausted[0] || playerExhausted[1])
                        {
                            state = STATE_GAMEOVER;
                        }
                        else
        ...
        switch (state)
        {
            ...
            case STATE_LEADERBOARD:
            {
                ...
                DrawTextEx(uiFont, "RANK", (Vector2){215, 120}, 22, 2, popYellow);
                DrawTextEx(uiFont, "PLAYER", (Vector2){340, 120}, 22, 2, popYellow);
                DrawTextEx(uiFont, "POINTS", (Vector2){660, 120}, 22, 2, popYellow);
                ...
                for (int i = 0; i < leaderboardCount; i++)
                {
                    ...
                    DrawText(TextFormat("%d", i + 1), 235, y, 22, rowColor);
                    DrawText(leaderboard[i].name, 340, y, 22, rowColor);
                    DrawText(TextFormat("%d", leaderboard[i].points), 680, y, 22, rowColor);
                }
